<template>
  <div>
    <base-list ref="list" :action="action" :query.sync="query">
      ...
      <el-table-column label="代理类型" prop="proxyType" width="100" align="center" />
      <el-table-column label="代理级别" prop="proxyLevel" width="100" align="center" />
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="{row}">
          <el-button type="text" @click="showProcessor(row)">{{ actionNames[row.finish] }}</el-button>
        </template>
      </el-table-column>
    </base-list>
    ...
    <el-dialog title="处理试用客户" :visible.sync="editorVisible" width="500px">
      <el-form :model="currentRow" label-position="left" label-width="70px">
        <el-form-item label="备注：" prop="remark">
          <template v-if="!!currentRow.finish">
            {{ currentRow.remark }}
          </template>
          <template v-else>
            <el-input
...
export default {
  ...
  methods: {
    update() {
      this.$refs.remarkForm.validate(valid => {
        if (valid) {
          ...
          updateCopartnerCustomer({ id, remark, followUp }).then(() => {
    ...
    showProcessor(row) {
      ...
      this.selectRow = this.$utils.deepcopy(row)
    }
  }
}
</script>
